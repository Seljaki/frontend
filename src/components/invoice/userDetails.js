...
function UserDetails({user, type}){
  ...
  return(
    <Grid item xs={5.5} sx={{ border: 'solid 1px', borderColor: myTheme.palette.secondary.main, borderRadius: 2, p:1}}>
      ...
      <Typography sx={{wordWrap: 'break-word'}}><span style={{ color: myTheme.palette.primary.main }}>Address: </span>{user.address}</Typography>
      <Typography sx={{wordWrap: 'break-word'}}><span style={{ color: myTheme.palette.primary.main }}>Phone: </span>{user.phone}</Typography>
